...
async function writeCloudState() {
  if (!firebaseSetDoc || !firebaseServerTimestamp || !cloudStateRef) return;
  try {
    await firebaseSetDoc(cloudStateRef, {
...
async function initCloudSync() {
  try {
    ...
    cloudStateRef = firestoreModule.doc(db, "kinepro", "shared-state");
    ...
    cloudEnabled = true;
    markCloudSynced();
    firebaseOnSnapshot(cloudStateRef, (remoteSnapshot) => {
...
function markSyncing() {
  ...
  syncTimer = setTimeout(() => {
    $("#sync-label").textContent = "Sincronizado";
    $("#sync-detail").textContent = "Datos persistentes disponibles";
...
function markCloudError() {
  $("#sync-label").textContent = "Modo local";
  $("#sync-detail").textContent = "Revise reglas/conexion de Firebase";
...
function getExerciseLibraryCategories() {
  return [...new Set(EXERCISE_LIBRARY.map((item) => item.category).filter(Boolean))].sort((a, b) => a.localeCompare(b));
}
...
function getFilteredExerciseLibrary() {
  ...
  return EXERCISE_LIBRARY
    ...
    .filter((item) => {
      ...
      return `${item.name} ${item.category} ${item.source}`.toLowerCase().includes(query);
    });
}

function findLibraryExerciseByName(name) {
  const normalized = String(name || "").trim().toLowerCase();
  return EXERCISE_LIBRARY.find((item) => item.name.toLowerCase() === normalized);
}
...
function useLibraryExercise() {
  ...
  const exercise = EXERCISE_LIBRARY[selectedIndex];
  if (!exercise) return;
...
function athleteLogin(event) {
  event.preventDefault();
  const dni = normalizeDni(new FormData(event.currentTarget).get("dni"));
...
function createAthleteAccess(patientId) {
  ...
  if (existing) {
    window.alert("Este deportista ya tiene acceso activo.");
    return;
  }
      ...
      <article class="entity-card">
        <header>
          <div>
            ...
            <div class="meta-line">DNI ${escapeHtml(access.dni)} &middot; ${access.active === false ? "inactivo" : "activo"}</div>
          </div>
    ...
    <article class="entity-card">
      <header>
        <div>
          ...
          <div class="meta-line">${routine.exercises.length} ejercicios &middot; ${escapeHtml(routine.goal || "Rutina asignada")}</div>
        </div>
        <span class="chip stable">asignada</span>
      </header>
      <div class="exercise-list">
        ${routine.exercises.map((exercise) => `
          <div class="exercise-row">
            <span>${escapeHtml(exercise.name)} <small>${formatExerciseDosage(exercise)}</small></span>
          </div>
        `).join("")}
      </div>
    </article>
...
function renderAthletePortal() {
  ...
  if (!patient) {
    ...
    $("#athlete-routines-list").innerHTML = "<div class='empty-state'>No se encontro una ficha vinculada a este DNI.</div>";
    $("#athlete-history-list").innerHTML = "";
    return;
  }

  const stats = getPatientStats(patient.id);
  $("#athlete-name").textContent = patient.name;
  $("#athlete-phase").textContent = patient.phase || "Seguimiento activo";
  $("#athlete-dni-chip").textContent = `DNI ${patient.dni || currentSession.dni}`;
  $("#athlete-phone").textContent = patient.phone || "-";
  $("#athlete-email").textContent = patient.email || "-";
  $("#athlete-routine-count").textContent = stats.routines.length;
  $("#athlete-session-count").textContent = stats.sessions.length;
  $("#athlete-notes").textContent = patient.notes || patient.history || "Sin observaciones cargadas.";
  ...
  $("#athlete-history-list").innerHTML = stats.sessions.map((item) => {
    ...
    `;
  }).join("") || "<div class='empty-state'>Todavia no tenes sesiones registradas.</div>";
}
...
function renderPatients() {
  ...
  $("#patients-list").innerHTML = list.map((patient) => {
    ...
    return `
      <article class="entity-card">
        <header>
          ...
          <span class="chip ${access ? "stable" : "controlled"}">${access ? "deportista" : "sin acceso"}</span>
        </header>
        <div class="stats-row">
        ...
        <div class="button-row">
          <button class="primary-action" data-open-patient="${patient.id}" type="button">Abrir perfil</button>
    ...
    `;
  }).join("") || "<div class='empty-state'>No hay pacientes para mostrar.</div>";
}
...
function renderRoutines() {
  ...
  $("#routines-list").innerHTML = list.map((routine) => {
    ...
    `;
  }).join("") || "<div class='empty-state'>No hay rutinas guardadas.</div>";
}
...
function renderHistory() {
  ...
  $("#history-list").innerHTML = list.map((item) => {
    ...
    `;
  }).join("") || "<div class='empty-state'>Todavia no hay sesiones cerradas.</div>";
}

function renderClinicalMetrics() {
  const completedExercises = state.history.reduce((sum, item) => sum + item.completed, 0);
  const totalExercises = state.history.reduce((sum, item) => sum + item.totalExercises, 0);
  const totalVolume = state.history.reduce((sum, item) => sum + item.volume, 0);
  const totalDuration = state.history.reduce((sum, item) => sum + item.duration, 0);
  const completion = totalExercises ? Math.round((completedExercises / totalExercises) * 100) : 0;
  const avgDuration = state.history.length ? Math.round(totalDuration / state.history.length) : 0;
  const patientCounts = state.history.reduce((counts, item) => {
  ...
  $("#metric-insights").innerHTML = `
    ...
    <article><strong>${state.history.length}</strong><span>sesiones cerradas</span></article>
  `;
}
...
function buildRoutinePrint(routineId) {
  ...
  if (!routine) return "";
  const patient = getPatient(routine.patientId);
  const date = new Intl.DateTimeFormat("es-PY", { day: "2-digit", month: "short", year: "numeric" }).format(new Date());
  ...
  return `
    <section class="print-sheet">
      <header>
        ...
        <div class="print-title">
          ...
          <div class="print-meta">
            <div>Nombre: ${escapeHtml(patient?.name || "-")}</div>
            <div>Objetivo: ${escapeHtml(routine.name)}</div>
            <div>Fecha: ${escapeHtml(date)}</div>
          </div>
        </div>
      </header>
      <div class="print-day">DIA <span>1</span></div>
      ...
      <div class="print-note">
        ...
        <span>${escapeHtml(routine.goal || patient?.notes || "Sin observaciones.")}</span>
      </div>
    </section>
  `;
}
...
function finishSession(sessionId) {
  ...
  const volume = session.exercises.reduce((sum, exercise) => {
    const numericLoad = Number.parseFloat(String(exercise.load).replace(",", "."));
    return sum + exercise.sets * exercise.reps * (Number.isFinite(numericLoad) ? numericLoad : 1);
  }, 0);
...
function bindEvents() {
  ...
  ["#patient-search", "#routine-search", "#history-search"].forEach((selector) => {
