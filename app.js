...
function renderReadOnlyRoutine(routine, patient) {
  ...
  return `
    <details class="entity-card athlete-fold athlete-routine-card">
      <summary class="athlete-summary">
        ...
        <div class="athlete-summary-actions">
          <button class="primary-action" data-athlete-start-routine="${routine.id}" type="button">Iniciar rutina</button>
...
function renderAthletePortal() {
  ...
  const activeSession = state.activeSessions.find((session) => session.source === "athlete" && session.patientId === patient.id);
  ...
  $("#athlete-profile-card").innerHTML = `
    <details class="panel athlete-fold athlete-profile-fold">
      <summary class="athlete-summary">
        <div>
      ...
      <div class="athlete-readonly-block">
        <span>Historial</span>
        <p>${escapeHtml(patient.history || "Sin antecedentes cargados.")}</p>
      </div>
      <div class="athlete-readonly-block">
        <span>Observaciones</span>
        <p>${escapeHtml(patient.notes || "Sin observaciones cargadas.")}</p>
      </div>
    </details>
  `;
  ...
  $("#athlete-history-panel").innerHTML = `
    <details class="panel athlete-fold athlete-history-fold">
      <summary class="athlete-summary">
        <div>
...
function renderAthleteActiveSession(session) {
  ...
  return `
    ...
    <div class="exercise-list athlete-execution-list">
      ${session.exercises.map((exercise) => `
        <article class="athlete-execution-row ${exercise.done ? "done" : ""}">
          <div class="athlete-exercise-head">
            <div>
              <h3>${escapeHtml(exercise.name)}</h3>
              <div class="meta-line">Plan: ${formatExerciseDosage(exercise)}</div>
            </div>
            <button class="${exercise.done ? "secondary-action" : "primary-action"}" data-athlete-toggle-exercise="${session.id}:${exercise.id}" type="button">
...
function finishSession(sessionId) {
  ...
  state.history.unshift(buildSessionHistoryRecord(session));
...
function startAthleteRoutine(routineId) {
  ...
  const existing = state.activeSessions.find((session) => (
    session.source === "athlete" &&
...
function toggleAthleteExercise(sessionId, exerciseId) {
  const session = state.activeSessions.find((item) => item.id === sessionId && item.source === "athlete");
  const exercise = session?.exercises.find((item) => item.id === exerciseId);
  if (!exercise) return;
  exercise.done = !exercise.done;
  persist();
